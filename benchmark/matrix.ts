...
const simpleSchema = { a: UINT8, b: UINT16, c: UINT32, d: INT8, e: INT16, f: INT32, g: FLOAT };
const simpleData = { a: 255, b: 65535, c: 4294967295, d: -128, e: -32768, f: -2147483648, g: 123.456 };

const profileSchema = { userId: UINT32, nickName: STRING, isVip: BOOL, age: UINT8 };
const stateSchema = {
  users: [profileSchema],
  posts: [{ postId: UINT32, title: STRING, score: UINT16, authors: [profileSchema] }],
};
const user = { userId: 101, nickName: "ABC", isVip: true, age: 34 };
const post = { postId: 100, title: "Hello World!", score: 999, authors: [{ userId: 102, nickName: "DEF", isVip: false, age: 28 }] };

function build(n: number) {
  const users = [], posts = [];
  for (let i = 0; i < n; i++) { users.push(user); posts.push(post); }
  return { users, posts };
}
...
for (const s of scenarios) {
  for (const c of combos) {
    ...
    suite.add(`pack   ${s.name} [${c.label}]`, () => { pack(s.data, s.schema as any, c.opt); });
    suite.add(`unpack ${s.name} [${c.label}]`, () => { unpack(packed, s.schema as any, c.opt); });
  }
}
...
for (const s of scenarios) {
  ...
  suite.add(`pack   ${s.name} [datapack]`, () => { pack(s.data, s.schema as any, jsonOpt); });
  suite.add(`pack   ${s.name} [JSON    ]`, () => { JSON.stringify(s.data); });
  suite.add(`unpack ${s.name} [datapack]`, () => { unpack(packed, s.schema as any, jsonOpt); });
  suite.add(`unpack ${s.name} [JSON    ]`, () => { JSON.parse(json); });
}
